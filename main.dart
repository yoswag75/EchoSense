...
import 'package:web_socket_channel/web_socket_channel.dart'; // Using standard WebSockets
...
// ==========================================
// STATE MANAGEMENT (FastAPI WebSocket Client)
// ==========================================
class SoundWatchProvider extends ChangeNotifier {
  WebSocketChannel? _channel;
  bool isConnected = false;
  
  String hardwareState = "CONNECTING";
  String hardwareMessage = "Connecting to Cloud Server...";
  ...
  Future<void> connectToServer() async {
    ...
    // =====================================================================
    // HARDCODED CLOUD SERVER URL
    // Replace this placeholder with your actual Render/Railway/Heroku URL
    // Make sure it starts with 'wss://' and ends with '/ws'
    // =====================================================================
    String wsUrl = 'wss://nlp-echosense-backend.onrender.com/ws'; 
    ...
    try {
      ...
      // Await the ready state to guarantee the server actually acknowledged the connection
      await _channel!.ready;
      
      isConnected = true;
      updateStatus("IDLE", "Connected to API Server");

      // Listen for incoming ML results from FastAPI
      _channel!.stream.listen((message) {
        try {
          final decoded = jsonDecode(message);
          
          if (decoded['event'] == 'hardware_status') {
            updateStatus(decoded['data']['state'], decoded['data']['msg']);
          } else if (decoded['event'] == 'sound_event') {
            _handleRealTimeEvent(
              decoded['data']['category'], 
      ...
      onError: (error) {
        isConnected = false;
        updateStatus("DISCONNECTED", "Failed to connect to server.");
      });
      
    } catch (e) {
      isConnected = false;
      updateStatus("DISCONNECTED", "Server offline or unreachable.");
    }
  }
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    // Automatically connect to the hardcoded server when the app starts
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<SoundWatchProvider>(context, listen: false).connectToServer();
    });
  }
